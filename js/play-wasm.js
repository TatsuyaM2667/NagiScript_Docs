...
(function () {
        ...
        get(_t, name) {
          ...
          switch (name) {
            ...
            case "__ngs_println_i64":
              return function (v) {
                logOutput(String(BigInt.asIntN(64, BigInt(v))));
              };
            case "__ngs_print_f64":
            case "__ngs_println_f64":
              return function (v) {
                logOutput(String(Number(v)));
              };
            case "__ngs_print_bool":
            case "__ngs_println_bool":
              return function (v) {
                logOutput(String(Boolean(v)));
              };
            default:
              return function () {
                /* noop host */
  ...
  function writeString(str) {
    ...
    for (let i = 0; i < bytes.length; i++)
      new Uint8Array(memory.buffer, ptr + 8 + i, 1)[0] = bytes[i];
  ...
  const mem = {
    allocated: 0,
    _need(n) {
      return memory.buffer.byteLength - this.allocated < n;
    },
    growToFit(n) {
      if (this._need(n)) {
  ...
  function parseArgs(str) {
    // "10, 20" / "42" → [10, 20] / 数値でない引数があれば null
    const parts = str
      .split(",")
      .map(function (s) {
        return s.trim();
  ...
  function tryRunMain(codeText) {
    ...
    if (
      /export "C"\s+fn\s+(fibonacci|factorial|gcd|is_prime|type_size)/.test(
        stripped,
      )
    ) {
  ...
  function isNgsCode(lang) {
    return (
      lang === "ngs" || lang === "nagiscript" || /\.ngs$/i.test(lang || "")
    );
  }

  // Run ボタンを付けるのは、デモ関数が定義・呼び出しされている
  // NagiScript ブロックだけにする（全ブロックに付けない）。
  function isRunnableDemo(codeText) {
    const stripped = codeText
      .replace(/\/\/[^\n]*/g, " ")
      .replace(/\/\*[\s\S]*?\*\//g, " ");
    const alt = Object.keys(EXPORT_FNS).join("|");
    const defRe = new RegExp('export\\s+"C"\\s+fn\\s+(' + alt + ")\\b");
    if (defRe.test(stripped)) return true;
  ...
  function attachRunButtons() {
    ...
    document
      .querySelectorAll("pre:not(.run-output):not(.demo-code)")
      .forEach(function (pre) {
        ...
        // デモ関数を含まないブロックには Run ボタンを付けない
        if (!isRunnableDemo(code.textContent)) return;
        ...
        status.innerHTML =
          '<i data-lucide="loader" class="spin-icon"></i> WASM 連携準備中…';
        ...
        // header に Run ボタンとステータスを差し込む（右寄せ）
        ...
        runBtn.addEventListener("click", function () {
          ...
          runBtn.innerHTML =
            '<i data-lucide="loader" class="spin-icon"></i> <span>実行中…</span>';
          if (window.lucide) window.lucide.createIcons();
          ...
          load()
            .then(function () {
              return Promise.resolve()
                ...
                .then(function () {
                  ...
                  status.innerHTML =
                    '<i data-lucide="check-circle"></i> ✅ WASM 実行 0ms';
            ...
            .catch(function (err) {
              ...
              runBtn.innerHTML = oldHtml;
              status.innerHTML =
                '<i data-lucide="alert-triangle"></i> ⚠ 実行失敗';
              status.style.display = "inline-flex";
    ...
    const run = function () {
      ...
      let args =
        meta.args === 1
          ...
          : demoInput.value.split(",").map(function (s) {
              return parseInt(s, 10) || 0;
            });
      if (meta.args === 2 && args.length < 2) args = [args[0] || 0, 1];
      const out = fns[name].apply(null, args);
      demoAnswer.innerHTML =
        formatValue(out) + "<small>" + (meta.args === 1 ? "" : "") + "</small>";
      const params = Array(meta.args)
        .map(function (_, i) {
          return "a" + i + ": i32";
        })
        .join(", ");
    ...
    load()
      .then(function (fns) {
        ...
        document.dispatchEvent(
          new CustomEvent("wasmReady", { detail: { exports: fns } }),
        );
      })
      .catch(function (err) {
